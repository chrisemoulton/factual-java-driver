...
public class UrlUtil {
	 ...
	 * @param param a map of parameter keys and values
	...
	private static String toUrl(String root, String parameters) {
	...
	private static String urlPair(Object name, Object val, boolean urlEncode) {
		if (val != null) {
			return name
					+ "="
					+ ((urlEncode && val instanceof String) ? UrlUtil
							.urlEncode(val.toString()) : val);
		} else {
			return null;
		}
	}
	 ...
	 * Convert parameters to a list of parameter strings
	 */
	private static List<String> toParamList(Map<String, Object> paramMap, boolean urlEncode) {
		List<String> paramList = Lists.newArrayList();
		for (Entry<String, Object> entry : paramMap.entrySet()) {
			paramList.add(UrlUtil.urlPair(entry.getKey(), String.valueOf(entry.getValue()), urlEncode));
		}
		return paramList;
	}
